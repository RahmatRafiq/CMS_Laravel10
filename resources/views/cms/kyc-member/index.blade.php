            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        @include('cms.member-status.form')
                    </div>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header d-block">
                        <h4 class="card-title">KYC Member Status</h4>
                    </div>
                    <div class="card-body">
                        <button type="button" class="btn btn-primary" onclick="form_data('add')" data-toggle="modal"
                            data-target=".form-data">Tambah KYC Member</button>
                    </div>
                </div>
            </div>
            <div class="col-lg-12">
                                ...
                                <tbody>
                                    @foreach ($data as $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->username }}</td>
                                            <td>
                                                <span class="badge light {{$item->is_kyc ? 'badge-success' : 'badge-danger'}}">
                                                    <i class="fa fa-circle {{$item->is_kyc ? 'text-success' : 'text-danger'}} mr-1"></i>
                                                    {{$item->is_kyc ? 'Active' : 'Non Active'}}
                                                </span>
                                            </td>
                                            <td>
                                                <div class="d-flex">
        ...
        <script>
            function form_data(tipe, data = false) {
                if (tipe == 'add') {
                    ...
                    $('#form-data-title').html('Tambah Data KYC Member Status');
                    }
                    else {
                        ...
                        $('#form-data-title').html('Edit Data KYC Member Status');
                        document.getElementById('form-data-form').action = "/manage-data/kyc-member-status/update/" + setting.id;
                    ...
                    function hapus(id) {
                        Swal.fire({
                            ...
                            text: "Akan Menghapus Data KYC Member Status !",
                            icon: 'warning',
